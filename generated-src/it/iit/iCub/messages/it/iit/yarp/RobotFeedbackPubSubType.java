...
public class RobotFeedbackPubSubType implements us.ihmc.pubsub.TopicDataType<it.iit.iCub.messages.it.iit.yarp.RobotFeedback>
{
    ...
    @Override
   public void serialize(it.iit.iCub.messages.it.iit.yarp.RobotFeedback data, us.ihmc.pubsub.common.SerializedPayload serializedPayload) throws java.io.IOException
   {
   ...
   @Override
   public void deserialize(us.ihmc.pubsub.common.SerializedPayload serializedPayload, it.iit.iCub.messages.it.iit.yarp.RobotFeedback data) throws java.io.IOException
   {
	...
	public final static int getCdrSerializedSize(it.iit.iCub.messages.it.iit.yarp.RobotFeedback data, int current_alignment)
	{
	    ...
	    current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);
	    for(int a = 0; a < data.getJointStates().size(); ++a)
	    {
	        current_alignment += it.iit.iCub.messages.it.iit.yarp.JointStatePubSubType.getCdrSerializedSize(data.getJointStates().get(a), current_alignment);}

	    current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);
	    for(int a = 0; a < data.getImuStates().size(); ++a)
	    {
	        current_alignment += it.iit.iCub.messages.it.iit.yarp.IMUStatePubSubType.getCdrSerializedSize(data.getImuStates().get(a), current_alignment);}

	    current_alignment += 4 + us.ihmc.idl.CDR.alignment(current_alignment, 4);
	    for(int a = 0; a < data.getForceSensors().size(); ++a)
	    {
	        current_alignment += it.iit.iCub.messages.it.iit.yarp.ForceSensorPubSubType.getCdrSerializedSize(data.getForceSensors().get(a), current_alignment);}
   ...
   public static void write(it.iit.iCub.messages.it.iit.yarp.RobotFeedback data, us.ihmc.idl.CDR cdr)
   {
	    ...
	    cdr.write_type_e(data.getForceSensors());else
